...
def bodycomp(mass, tbw, method="reilly", simulate=False, n_rand=1000):
    ...
    if method == "reilly":
        if simulate is True:
            ...
            bc["tba"] = 0.1 - (0.008 * mass) + (0.05 * tbw) + rnorm(0, 0.3, n_rand)
            bc["tbge"] = (40.8 * mass) - (48.5 * tbw) - 0.4 + rnorm(0, 17.2, n_rand)
        else:
            bc["ptbw"] = 100 * (tbw / mass)
        ...
        raise ValueError(
            "`method` must be either `reilly` or `gales`, not " "`{}`".format(method)
        )
...
def lip2dens(
    perc_lipid, dens_lipid=0.9007, dens_prot=1.34, dens_water=0.994, dens_ash=2.3
):
...
def dens2lip(
    dens_gcm3, dens_lipid=0.9007, dens_prot=1.34, dens_water=0.994, dens_ash=2.3
):
...
def buoyant_force(dens_gcm3, vol, sw_dens=1.028):
    ...
    return (1000 * (sw_dens - dens_gcm3)) * (1e-6 * vol) * g


def diff_speed(
    sw_dens=1.028, dens_gcm3=1.053, seal_length=300, seal_girth=200, Cd=0.09
):
